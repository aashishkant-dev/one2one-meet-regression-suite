...
(async () => {
  ...
  // click the flag/country-code button next to Event Contact Number
  const label = page.getByText('Event Contact Number', { exact: true });
  const flagBtn = label.locator('xpath=following::button[1]');
  await flagBtn.click();
  await page.waitForTimeout(500);
  await page.screenshot({ path: OUT + '/38-flag-dropdown.png', fullPage: true });
  await browser.close();
})();
