...
(async () => {
  ...
  console.log('In delegate mode:', org.url());

  // TC-EOT-004: try to hit an organizer-only URL while in delegate mode
  await org.goto(BASE + '/organizer/settings');
  await org.waitForTimeout(2000);
  console.log('URL after visiting /organizer/settings while in delegate mode:', org.url());

  // now go back to delegate dashboard and toggle off
  await org.goto(BASE + '/delegate/booking-test-event-6a78985de09a0/dashboard');
  await org.waitForTimeout(2000);
  await org.getByRole('button', { name: 'Set delegate inactive' }).click();
  await org.waitForTimeout(2500);
  console.log('URL after toggling back off:', org.url());
  await browser.close();
})();
