...
(async () => {
  ...
  const inputs = await org.locator('input').evaluateAll(els =>
    els.map(e => ({ type: e.getAttribute('type'), id: e.id, aria: e.getAttribute('aria-label'), value: e.value, checked: e.checked }))
  );
